...
{
	public partial class ChangeEmployeeStatus : Form
	{
		#region Shadow
		...
		BO.Employee _oEmployee = new BO.Employee();
		...
		public ChangeEmployeeStatus(UserControl caller)
		{
			InitializeComponent();
			callingForm = caller;
		}
		...
		public void EditEmployee(BO.Employee oEmployee)
		{
			...
			if (_oEmployee != null)
			{
				status = _oEmployee.Status.ToString();
				txt_User.Text = "(" + _oEmployee.EmployeeNo + ") " + _oEmployee.Name;
				statusString.Text = status.ToUpper();

				switch (_oEmployee.Status)
				{
		...
		private void ChangeStatus_Click(object sender, EventArgs e)
		{
			...
			if (_oEmployee.Status == newStatus)
			{
				MessageBox.Show("This is employee's previous status.\nPlease choose another status.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				return;
			}
			_oEmployee.Status = newStatus;
			//_oEmployee.StatusChangedDate = DateTime.Now;
			_oEmployee.ModifiedBy = oCurrentUser.ID;
			_oEmployee.ModifiedDate = DateTime.Now;

			try
			{
				string result = employeeService.UpdateUserStatus(_oEmployee.ID, newStatus, oCurrentUser.ID, DateTime.Now);

				if (result == "Ok")
				{
					MessageBox.Show($"Employee status changed successfully.\n{_oEmployee.Name} is now {newStatus.ToString()}", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
				}
				else
				{
